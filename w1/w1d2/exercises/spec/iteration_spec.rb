...
describe "#benchmark" do
  ...
  it "calls the given block `count` times" do
    expect do |block|
      benchmark(3, &block)
  ...
  it "returns a number" do
    expect(benchmark {}).to be_instance_of(Float)
  end
end

describe "#race" do
  let(:tortoise) do
    Proc.new { 100.times { 2**32 } }
  end

  let(:hare) do
    Proc.new { 1.times { 2**32 } }
  end

  let(:procs) { [tortoise, hare] }

  it "returns the faster proc" do
    expect(race(100, procs)).to be(hare)
  end
end
